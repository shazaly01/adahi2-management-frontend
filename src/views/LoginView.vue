...
const handleLogin = async () => {
  ...
  errorMessage.value = ''
  try {
    // 1. استدعاء دالة login وانتظار التحقق من الحساب
    await authStore.login(credentials)

    // 2. فحص التوجيه الذكي بناءً على الصلاحيات
    if (authStore.returnUrl) {
      // إذا كان هناك رابط مخزن مسبقاً حاول المستخدم دخوله
      router.push(authStore.returnUrl)
    } else if (authStore.can('distribution.deliver') && !authStore.can('user.view')) {
      // إذا كان المستخدم موزعاً ميدانياً (يمتلك صلاحية التسليم ولا يملك صلاحية إدارة المستخدمين)
      // يتم توجيهه فوراً لشاشة التسليم المستقلة المتوافقة مع الهاتف
      router.push('/delivery')
    } else {
      // المدراء، الـ Super Admin، وباقي موظفي النظام يتوجهون كالمعتاد للوحة التحكم الرئيسية
